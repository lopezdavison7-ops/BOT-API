// lib/nsfwDetect.js — 🔞 Detector NSFW con análisis por URL pública
import fs from 'fs';
...
// ---------- ANALIZAR IMAGEN (con URL pública y múltiples formatos) ----------
export async function analizarImagen(buffer, mime) {
    console.log('[NSFW-API] Intentando analizar imagen de', (buffer.length / 1024).toFixed(1), 'KB');

    // PASO 1: Subir a telegraph para obtener URL pública
    let urlPublica = null;
    try {
        const form = new FormData();
        form.append('file', new Blob([buffer], { type: mime || 'image/jpeg' }), 'img.jpg');
        const r = await fetch('https://telegra.ph/upload', { 
            method: 'POST', 
            body: form, 
            signal: AbortSignal.timeout(15000) 
        ...
        if (Array.isArray(j) && j[0]?.src) {
            urlPublica = 'https://telegra.ph' + j[0].src;
            console.log('[NSFW-API] URL pública:', urlPublica);
        }
    } catch (e) {
        console.error('[NSFW-API] Error subiendo a telegraph:', e.message);
    }

    if (!urlPublica) {
        console.log('[NSFW-API] No se pudo subir a telegraph, usando multipart directo');
        
        // Fallback: intentar multipart directo
        for (const campo of ['image', 'file', 'img']) {
            try {
                ...
                if (json && json.status === true && json.analysis) {
                    console.log('[NSFW-API] ✅ Éxito con multipart campo:', campo);
                    return json;
                }
            } catch (e) {
                console.error('[NSFW-API] Multipart campo', campo, 'falló:', e.message);
            }
        }
        return null;
    }

    // PASO 2: Probar diferentes formatos con la URL
    const intentos = [
        {
            nombre: 'JSON {url}',
    ...
    for (const intento of intentos) {
        try {
            console.log('[NSFW-API] Intentando:', intento.nombre);
            const res = await intento.fn();
            const texto = await res.text();
            console.log('[NSFW-API]', intento.nombre, 'HTTP', res.status);
            ...
            if (res.ok) {
                try {
                    ...
                    console.log('[NSFW-API] Respuesta no es JSON válido');
                }
            }
        } catch (e) {
            console.error('[NSFW-API]', intento.nombre, 'falló:', e.message);
        }
    }
