        ...
        try {
            ...
            if (categoriaPedida) {
                ...
                if (categorias[catNormal]) {
                    return await enviarMenuCategoria(sock, jid, msg, catNormal, categorias[catNormal], prefijo, autor);
                }
            }
            ...
            // MENCION SIMPLE EN EL TEXTO - SIN ROMPER
            const headerText = `┏━━━ ⋆⋅☆⋅⋆ ━━━┓
   🌸 *${botName}* 🌸
┗━━━ ⋆⋅☆⋅⋆ ━━━┛

👋 ¡Hola @${autor?.num || 'usuario'}! ✨
...
╰──────────────`;
            ...
            try {
                if (VIDEO_MENU_URL) {
                    const media = await prepareWAMessageMedia({ video: { url: VIDEO_MENU_URL }, gifPlayback: false }, { upload: sock.waUploadToServer });
                    header = { title: '🌸 MENÚ PRINCIPAL 🌸', hasMediaAttachment: true, videoMessage: media.videoMessage };
                } else if (fs.existsSync(FOTO_MENU)) {
                    const media = await prepareWAMessageMedia({ image: { url: FOTO_MENU } }, { upload: sock.waUploadToServer });
            ...
            const msgSend = generateWAMessageFromContent(
                ...
                { userJid: sock.user.id, quoted: msg, mentions: [autor?.jid] } // <-- MENCION AQUI
            );
...
async function enviarMenuCategoria(sock, jid, msg, categoria, comandos, prefijo, autor) {
